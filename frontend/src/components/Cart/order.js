        ...
        <div className="py-4 px-2 flex flex-wrap flex-shrink justify-between">
          <aside className="w-full xl:w-9/12 p-4">
            ...
            <div>
              <ul id="progressbar" className="flex w-full justify-center mb-12">
                <li className="active w-1/3 text-center " id="account">
                  <strong>Account</strong>
                </li>
                <li className="active w-1/3 text-center" id="shipping">
                  <strong>Shipping</strong>
                </li>
                <li className="active w-1/3 text-center" id="confirm">
                  <strong>Place Order</strong>
                </li>
              </ul>
            </div>
            <div className="px-8 my-2 mb-4">
              <p className="font-bold text-lg text-gray-900 text-center w-full my-2">
                Delivery Details
              </p>
            ...
            <div className=" px-8 my-2 mb-4">
              ...
              <CartItem
                place={true}
                items={props.cartItems && props.cartItems}
              />
            </div>
          </aside>
          <section className="w-full xl:w-3/12 my-8 xl:my-6">
            ...
            <CartCheckout
              ...
              qty={props.cartItems && props.cartItems.length}
              total={props.cart && props.cart.totalPrice}
              items={props.cart && props.cart.totalPrice}
              shipping={0}
              tax={0}
            >
              {button ? (
                <button
                  ...
                  className={`w-full mt-4 flex justify-center border border-gray-200 rounded py-4 px-4 leading-tight focus:outline-none focus:bg-white focus:text-black focus:border-gray-500 hover:bg-white hover:text-black hover:border-gray-500 uppercase ${
                    props.loading
                ...
                >
                  {props.loading ? (
                    <>
                      <img
                        src={loader}
                        className="h-6 w-10 px-2 fill-current"
                        alt="loading..."
                      />
                      loading &nbsp;...
                    </>
                  ...
                  createOrder={(data, actions) => {
                    return actions.order.create({
                      purchase_units: [
                        {
                          amount: {
                            value: Number(props.cart.totalPrice),
                          },
                        },
                      ],
                    });
                  }}
                  onError={(err) => console.log(err,"1444")}
                  onApprove={() => {
                    sendOrder(props.cart);
                  }}
                  className={`w-full mt-4 flex justify-center border border-gray-200 rounded py-4 px-4 leading-tight focus:outline-none focus:bg-white focus:text-black focus:border-gray-500 hover:bg-white hover:text-black hover:border-gray-500 uppercase ${
                    props.loading
                      ? "bg-gray-200  text-black cursor-not-allowed"
